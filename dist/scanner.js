...
const DEFAULT_INDEX = process.env.DSCAN_INDEX ?? "E:/workspace/deepseek/work/dshbase-plugin-directory/dshbase-directory.json";
...
export function findPlugin(name, plugins) {
    const q = name.trim().toLowerCase();
    if (!q)
        return null;
    const exact = plugins.find((p) => p.name.toLowerCase() === q || (p.slug ?? "").toLowerCase() === q);
    if (exact)
        return exact;
    const fuzzy = plugins
...
export function parseCmdToRepo(cmd) {
    if (!cmd)
        return null;
...
export function normalizeRemote(raw) {
    ...
    if (s.startsWith("github:"))
        return `https://github.com/${s.slice("github:".length)}.git`;
    ...
    const walk = (dir) => {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (TEXT_EXTS.has(ext) || SPECIAL_FILES.has(entry.name))
                    out.push(full);
            }
        }
    };
...
export function scanText(text, fileLabel, source = "static") {
    const findings = [];
    ...
    for (const rule of RULES) {
        for (let i = 0; i < lines.length; i++) {
            ...
            if (rule.id !== "R008" && COMMENT_RE.test(line))
                continue;
            // R002 不命中 eval 的函数定义（def eval / function eval 是 ML 代码里
            // 常见的 evaluate 命名，不是动态执行）。
            if (rule.id === "R002" && /^\s*(?:async\s+)?def\s+eval\b|^\s*(?:export\s+)?(?:async\s+)?function\s+eval\b/.test(line))
                continue;
            ...
            for (const key of lifecycle) {
                ...
                if (script && script.trim()) {
                    ...
                    findings.push({
                        ...
                        evidence,
                        recommendation: "Review lifecycle scripts carefully; they run automatically during install. Prefer zero-install packages or pinned, reviewed scripts.",
                        source,
                        rule: "R009",
                    });
                }
...
function isClientFile(fileLabel) {
    const lower = fileLabel.toLowerCase();
    const base = basename(lower);
    return (base === "client.mjs" ||
        base === "client.js" ||
        base === "client.ts" ||
...
function isProfileConfigFile(fileLabel) {
    const lower = fileLabel.toLowerCase();
    const base = basename(lower);
    return (base === "cordis.yml" ||
...
function scanDshSpecific(fileLabel, text, source = "static") {
    ...
    if (/cordis\.patch\.ya?ml$/i.test(lowerFile) || basename(lowerFile) === "cordis.patch.yml") {
        ...
        if (hasInsert && dangerousNameHit) {
            findings.push(buildDshFinding(DSH_SPECIFIC_RULES[0], `${fileLabel}: contains '- insert:' of ${dangerousNameHit}`, source));
        }
        ...
        for (const id of securityRowIds) {
            const disableRe = new RegExp(`id:\\s*['"]?${escapeRegExp(id)}['"]?[\\s\\S]{0,200}?disabled:\\s*true`, "i");
            const disableRe2 = new RegExp(`${escapeRegExp(id)}[\\s\\S]{0,80}?disabled:\\s*true`, "i");
            if (disableRe.test(text) || disableRe2.test(text)) {
                findings.push(buildDshFinding(DSH_SPECIFIC_RULES[0], `${fileLabel}: disables security row '${id}'`, source));
            }
        }
    }
    ...
    if (isClientFile(fileLabel)) {
        ...
        for (const p of clientPatterns) {
            ...
            if (m) {
                findings.push(buildDshFinding(DSH_SPECIFIC_RULES[1], `${fileLabel}: ${p.desc} (${m[0].slice(0, 120)})`, source));
            }
        }
    }
    ...
    if (isProfileConfigFile(fileLabel) || (isCodeFile(fileLabel) && /(?:\.dsh|profiles|cordis\.patch\.ya?ml)/i.test(text))) {
        ...
        for (const p of tamperPatterns) {
            if (p.re.test(text)) {
                findings.push(buildDshFinding(DSH_SPECIFIC_RULES[2], `${fileLabel}: ${p.desc}`, source));
            }
        }
    }
    return findings;
}
export function scanLocalPath(localPath) {
    ...
    if (statSync(resolved).isDirectory()) {
        ...
        for (const file of files) {
            ...
            if (lower.startsWith("readme"))
                readmeParts.push(text.slice(0, 12000));
        ...
        try {
            ...
            for (const file of files) {
                ...
                findings.push(...fileFindings);
                if (basename(file).toLowerCase().startsWith("readme"))
                    readmeParts.push(text.slice(0, 12000));
                if (fileFindings.some((f) => f.severity === "high" || f.severity === "critical")) {
                    codeExcerpts.push(`--- ${rel} ---\n${text.slice(0, 4000)}`);
                }
        ...
        if (basename(resolved).toLowerCase().startsWith("readme"))
            readmeParts.push(text.slice(0, 12000));
    }
    return {
        findings,
...
export function metadataFindings(meta) {
    const findings = [];
    ...
    if (!meta.verified) {
        findings.push({
            ...
            recommendation: "verified=false only means dshbase CI did not confirm installability, not that it is malicious. Treat as higher uncertainty and review source before installing.",
            source,
            rule: "M001",
        });
    }
    ...
    if (stars < 10) {
        findings.push({
            ...
            recommendation: "Low adoption increases supply-chain risk. Prefer well-known maintainers and inspect the repository directly.",
            source,
            rule: "M002",
        });
    }
    ...
    if (trust !== "gold" && trust !== "silver") {
        findings.push({
            ...
            recommendation: "Unknown trust tier means no strong community/vendor signal. Inspect repository owner and recent activity.",
            source,
            rule: "M004",
        });
    }
...
export function computeScore(findings, meta) {
    ...
    let score = 0;
    for (const w of byCategory.values())
        score += w;
    if (meta) {
        const trust = meta.trust ?? "";
        if (trust === "gold")
            score -= 5;
        else if (trust === "silver")
            score -= 2;
        else
            score += 3;
        if (meta.verified)
...
export function recommendationFromFindings(findings, score, severity) {
...
export function resolveTarget(raw, plugins, offline) {
...
export async function scanTarget(target, opts = {}) {
    ...
    if (target.kind === "dir" || target.kind === "zip" || target.kind === "md" || target.kind === "file") {
        ...
        if (opts.offline) {
            findings.push({
                ...
                recommendation: "Re-run without --offline to clone and statically scan the repository, or provide a local directory.",
                source: "static",
                rule: "E002",
            });
            sourceAvailable = false;
            ...
            try {
                try {
                    ...
                    findings.push({
                        ...
                        recommendation: "Check network access or clone manually into a local directory and scan that directory.",
                        source: "static",
                        rule: "E003",
                    });
                    sourceAvailable = false;
        ...
        findings.push({
            ...
            recommendation: "Install the package in a sandbox and scan its unpacked contents, or provide a local directory of the package.",
            source: "static",
            rule: "E004",
        });
        sourceAvailable = false;
        ...
        findings.push({
            ...
            evidence: target.raw,
            recommendation: "Provide a GitHub repo URL, local directory/zip, or a plugin name that exists in the dshbase index.",
            source: "static",
            rule: "E005",
        });
        sourceAvailable = false;
    ...
    const safeToInstall = !allFindings.some((f) => f.severity === "high" || f.severity === "critical") &&
        finalScore < 40;
